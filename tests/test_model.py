...
from rlcd.model import ActionCosts, RLCDConfig, RLCDModel, masked_softmax
...
def test_config_round_trips_through_a_dict() -> None:
    config = RLCDConfig(encoder_name="tiny", num_decision_layers=3, temperature=2.0)

    restored = RLCDConfig.from_dict(config.to_dict())
...
def test_config_rejects_invalid_values() -> None:
    with pytest.raises(ValueError, match="num_decision_layers"):
        RLCDConfig(num_decision_layers=0)
    with pytest.raises(ValueError, match="temperature"):
        RLCDConfig(temperature=0.0)
    with pytest.raises(ValueError, match="dropout"):
        RLCDConfig(dropout=1.0)


def test_model_rejects_a_hidden_size_without_room_for_heads() -> None:
    with pytest.raises(ValueError, match="hidden size"):
        RLCDModel(tiny_encoder(hidden_size=65), RLCDConfig(num_decision_layers=1))
