...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--encoder", default=RLCDConfig().encoder_name)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    model = RLCDModel.from_pretrained(RLCDConfig(encoder_name=args.encoder)).to(device)
    optimizer = AdamW(
        [
...
def _optimizer_step(model: RLCDModel, optimizer: AdamW, max_grad_norm: float) -> None:
