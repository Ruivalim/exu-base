...
class RLCDConfig:
    ...
    @classmethod
    def from_dict(cls, data: Mapping[str, object]) -> RLCDConfig:
...
class RLCDModel(nn.Module):
    """Encoder-only typed-decision model."""

    def __init__(self, encoder: PreTrainedModel, config: RLCDConfig | None = None) -> None:
        super().__init__()
    ...
    @classmethod
    def from_pretrained(cls, config: RLCDConfig | None = None) -> RLCDModel:
        """Load base encoder weights. The decision head starts untrained."""
        settings = config or RLCDConfig()
    ...
    def forward(
        ...
        """Score a batch emitted by :meth:`rlcd.sequence.SequenceBuilder.pad`."""
