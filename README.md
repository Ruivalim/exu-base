...
[![CI](https://github.com/ruivalim/rlcd-base/actions/workflows/ci.yml/badge.svg)](https://github.com/ruivalim/rlcd-base/actions/workflows/ci.yml)
...
RLCD means **Reinforcement Learning for Calibrated Decisions**. The reward is a
strictly proper scoring rule, so the only way to increase it is to report honest
probabilities. That is the whole trick: calibration is not coaxed out of the
model with a prompt, it is the optimum of the objective.
...
```python
from rlcd import DecisionQuestion, Option
...
```python
from rlcd import DecisionRuntime
...
The full walkthrough is in [docs/rlcd-guide.md](docs/rlcd-guide.md), and
...
distribution plus a separate confidence, and the idea of gating action on that
confidence. The "System One" framing is theirs.
