...
RLCD trains encoder-only models that answer a **typed question** about a **state**
...
The short version: the typed-decision contract comes from TypeSafe's Jev, the
open build recipe comes from Laya, and no source code was copied from either.

## Layout

```
src/rlcd/          the package, all of it (there is no second source tree)
  ...
  model.py         RLCDModel: encoder + type embedding + 2 layers + scorer + act head
  ...
  evaluate.py      the `rlcd-evaluate` CLI
  train.py         the `rlcd-train` CLI
...
`uv run python examples/end_to_end.py`, `uv run rlcd-train --help`.
...
| The build walkthrough, with the traps | `docs/rlcd-guide.md` |

The two walkthroughs are English/Portuguese pairs
(`rlcd-guide.{md,pt-BR.md}` and `usage-guide.{md,pt-BR.md}`). Keep them in step:
...
- Every public name is re-exported from `src/rlcd/__init__.py` with an `__all__`.
...
- Docs and the README are English. The Makefile help and
  `docs/rlcd-guide.pt-BR.md` are Portuguese on purpose; leave them.
