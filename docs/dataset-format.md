...
`family` exists for this, and `rlcd-evaluate` reports metrics per family so you
can see the gap between seen and unseen families.

## Augmentation

`rlcd-train --option-shuffle` permutes a question's options on every pass and
permutes the target with them, so position carries no information. Evaluate with
`rlcd-evaluate --order-permutations N` to measure whether it worked.
